<x-app-layout>
    ...
    <form action="{{ route('clients.update', $client->id) }}" method="POST" class="space-y-4">
        ...
        <div class="grid grid-cols-2">
            <div class="col-span-2 max-w-[560px]">
                <x-label for="password" :value="__('page.clients.form.password.label')"/>

                <x-input
                        type="text" class="w-full mt-1" name="password" value=""
                        placeholder="{{ __('page.clients.form.password.placeholder') }}"
                />
        ...
        <div class="grid grid-cols-2">
            <div class="col-span-2 max-w-[560px]">
                ...
                <x-select name="business_field_id" :display="$client->businessField->name"
                          :value="$client->businessField->id">
                    @foreach ($businessFields as $business_field)
                        <li
                                class="text-gray-800 text-sm hover:bg-slate-50 cursor-pointer select-none py-2 ps-3 pe-9"
                                business_field="option"
