<!-- begin::Dashboard -->
@can('view dashboard')
    <x-sidebar-link href="{{ route('admin.dashboard') }}" active="{{ request()->route()->named('admin.dashboard') }}">
...
@can('view subscriptions')
    <x-sidebar-link href="{{ route('subscriptions.index') }}" active="{{ request()->route()->named('subscriptions.index') || request()->route()->named('subscriptions.create') || request()->route()->named('subscriptions.edit') }}">
        ...
        <span class="block text-sm">{{ __('sidebar.subscriptions') }}</span>
    </x-sidebar-link>
@endcan
<!-- end::Subscriptions -->
...
@can('view halls')
    <x-sidebar-link href="{{ route('halls.index') }}" active="{{ request()->route()->named('halls.index') || request()->route()->named('halls.create') || request()->route()->named('halls.edit')}}">
        ...
        <span class="block text-sm">{{ __('sidebar.halls') }}</span>
    </x-sidebar-link>
@endcan
<!-- end::Halls -->
...
@can('view advertisements')
    <x-sidebar-link href="{{ route('advertisements.index') }}" active="{{ request()->route()->named('advertisements.index') || request()->route()->named('advertisements.create') || request()->route()->named('advertisements.edit')}}">
        ...
        <span class="block text-sm">{{ __('sidebar.ads') }}</span>
    </x-sidebar-link>
@endcan
<!-- end::Ads -->
...
@can('view clients')
    <x-sidebar-link href="{{ route('clients.index') }}" active="{{ request()->route()->named('clients.index') || request()->route()->named('clients.create') || request()->route()->named('clients.edit')}}">
        ...
        <span class="block text-sm">{{ __('sidebar.clients') }}</span>
    </x-sidebar-link>
@endcan
<!-- end::Clients -->
...
<!-- begin::Settings -->
@can('view settings')
    <x-sidebar-link href="{{ route('settings') }}" active="{{ request()->route()->named('settings') || request()->route()->named('packages.index') || request()->route()->named('features.index') || request()->route()->named('business-fields.index') }}">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
